...
class ContentView(TimeStampedModel):
    ...
    def __str__(self) -> str:
        return (
            ...
            f"{self.user.get_full_name if self.user else 'Anonymous'} from IP {self.viewer_ip} at {self.last_viewed}"
        )
    ...
    def record_view(
        ...
        try:
            content_view, created = cls.objects.get_or_create(
            ...
            if not created:
                content_view.last_viewed = timezone.now()
                content_view.save(update_fields=["last_viewed"])
