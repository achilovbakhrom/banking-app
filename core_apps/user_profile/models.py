...
class Profile(TimeStampedModel):
    ...
    date_of_birth = models.DateField(
        _("Date of Birth"), default=settings.DEFAULT_BIRTHDATE
    )
    ...
    id_issued_date = models.DateField(
        _("ID Issued Date"), default=settings.DEFAULT_DATE
    )
    ...
    signature_url = models.URLField(_("Signature URL"), blank=True, null=True)
        ...
        if self.id_issued_date and self.id_expiry_date:
            if self.id_expiry_date <= self.id_issued_date:
                raise ValidationError(
                    {
                        "id_expiry_date": _(
                            "ID Expiry Date must be after ID Issued Date."
                        )
                    }
                )
...
class NextOfKin(models.Model):
    ...
    def clean(self):
        super().clean()
        if self.is_primary:
            existing_primary = NextOfKin.objects.filter(
    ...
    def __str__(self) -> str:
        return f"{self.title} {self.first_name} {self.last_name} - Next of Kin for {self.profile.user.get_full_name()}"
